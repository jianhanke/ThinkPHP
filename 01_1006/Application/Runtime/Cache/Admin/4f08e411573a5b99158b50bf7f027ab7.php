...
<div class="table-box">
	<table>
        ...
        <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vol): $mod = ($i % 2 );++$i;?><volist name='vol' id='vo'>
            <tr>
                <td class="num"> <?php echo ($vol["id"]); ?> </td>
                <td class="name"><?php echo (str_repeat('$emsp;',$vol["level"]*2)); echo ($vol["name"]); ?> </td>
                <td class="process"><?php if($vol[pid]==0): ?>顶级部门<?php endif; ?>  </td>
